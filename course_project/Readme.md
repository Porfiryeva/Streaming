...
- Стек:
    ...
    - Cassandra, Kafka, Spark, Hadoop
    - библиотеки: pandas, pyspark

#### 1. Постановка задачи

- имеются исторические данные о трафике на участках дорог и данные о авариях (хранятся на HDFS);
- на этих данных обучается модель ML для предсказания аварийности (вероятности аварии) для конкретной точки, дня недели;
- на HDFS хранится справочная информация о участках дорог
- в потоке (Kafka) поступают сырые данные о текущем трафике на пунктах подсчёта;
